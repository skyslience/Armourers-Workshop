    ...
    public ContainerSkinWardrobe(InventoryPlayer invPlayer, ExPropsPlayerEquipmentData customEquipmentData) {
        ...
        for (int i = 0; i < ExPropsPlayerEquipmentData.MAX_SLOTS_PER_SKIN_TYPE; i++) {
            if (i < ewd.getUnlockedSlotsForSkinType(SkinTypeRegistry.skinHead)) {
                addSlotToContainer(new SlotSkin(SkinTypeRegistry.skinHead, headInv, i, 88 + i * 19, 18).setVisible(false));
                skinSlots += 1;
            }
            if (i < ewd.getUnlockedSlotsForSkinType(SkinTypeRegistry.skinChest)) {
                addSlotToContainer(new SlotSkin(SkinTypeRegistry.skinChest, chestInv, i, 88 + i * 19, 37).setVisible(false));
                skinSlots += 1;
            }
            if (i < ewd.getUnlockedSlotsForSkinType(SkinTypeRegistry.skinLegs)) {
                addSlotToContainer(new SlotSkin(SkinTypeRegistry.skinLegs, legsInv, i, 88 + i * 19, 56).setVisible(false));
                skinSlots += 1;
            }
            if (i < ewd.getUnlockedSlotsForSkinType(SkinTypeRegistry.skinFeet)) {
                addSlotToContainer(new SlotSkin(SkinTypeRegistry.skinFeet, feetInv, i, 88 + i * 19, 75).setVisible(false));
                skinSlots += 1;
            }
        }
        
        
        addSlotToContainer(new SlotSkin(SkinTypeRegistry.skinSword, swordInv, 0, 29, 113).setVisible(false));
        addSlotToContainer(new SlotSkin(SkinTypeRegistry.skinBow, bowInv, 0, 49, 113).setVisible(false));
        addSlotToContainer(new SlotSkin(SkinTypeRegistry.skinArrow, arrowInv, 0, 69, 113).setVisible(false));
        addSlotToContainer(new SlotSkin(SkinTypeRegistry.skinWings, wingInv, 0, 89, 113).setVisible(false));
        skinSlots += 4;
        
        for (int x = 0; x < 9; x++) {
            addSlotToContainer(new SlotHidable(invPlayer, x, 54 + 18 * x, 232).setVisible(false));
        }

        for (int y = 0; y < 3; y++) {
            for (int x = 0; x < 9; x++) {
                addSlotToContainer(new SlotHidable(invPlayer, x + y * 9 + 9, 54 + 18 * x, 174 + y * 18).setVisible(false));
            }
        }
    }
